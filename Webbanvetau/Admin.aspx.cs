        ...
        protected void btnthem_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cnn = new SqlConnection(conString))
            {
                using (SqlCommand cmd = new SqlCommand("spVetau_Insert", cnn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@makh", Int32.Parse(ddKhach.SelectedValue));
                    cmd.Parameters.AddWithValue("@matau", Int32.Parse(ddtau.SelectedValue));
                    cmd.Parameters.AddWithValue("@thanhtien", float.Parse(txtThanhtien.Text));
                    cmd.Parameters.AddWithValue("@matinhtrang", 1);
                    cmd.Parameters.Add("@ngayban", SqlDbType.DateTime);
                    cmd.Parameters["@ngayban"].Value = strNgayban;
                    cmd.Parameters.AddWithValue("@malichtrinh", Int32.Parse(hdmalichtrinh.Value));
                    cmd.Parameters.AddWithValue("@matoa", Int32.Parse(ddToa.SelectedValue));
                    cmd.Parameters.AddWithValue("@maghe", Int32.Parse(ddGhe.SelectedValue));
                    cmd.Parameters.Add("@ngaytra", SqlDbType.DateTime);
                    cmd.Parameters["@ngaytra"].Value = strNgaytra;
                    cnn.Open();
                    cmd.ExecuteNonQuery();
                }
                cnn.Close();
